...
router.post('/checkVersion', async (req, res) => {
  try {
    ...
    if (body.platform === 'ios' && body.version === '1.3.1.1')
      return res.json({ status: '200' });

    /* if (body.platform === 'ios' && body.version === '1.3.0') {
      return res.json({
    ...
    if (body.platform === 'android' && body.version === '1.3.1.1')
      return res.json({ status: '200' });

    /* if (body.platform === 'android' && body.version === '1.3.0') {
      return res.json({
        msg: `Our app is being updated to make the experience even better for you. Kindly try again in 24 hours.`,
      });
    } */
